"""
╔════════════════════════════════════════════════════════════════════════════════╗
║                     Hierarchical Self-Organizing Maps (HSOM)                   ║
║                 Building Multi-Scale Neural Feature Hierarchies                 ║
╚════════════════════════════════════════════════════════════════════════════════╝

Created by: Benedict Chen
Contact: github.com/benedictchen • benedict@benedictchen.com
If this helped with your research, consider buying me a coffee ☕ 
...or better yet, a research grant 💰
...or even better, a private jet ✈️ (for "research purposes" of course)
...
• Rauber et al. (2002) "The Growing Hierarchical Self-Organizing Map"
  - Introduced hierarchical SOM architecture with growing capabilities
  - Enabled multi-resolution data representation at different scales

• Kohonen (1982) "Self-Organized Formation of Topologically Correct Feature Maps"  
  - Foundation of self-organizing maps with neighborhood preservation
  - Established competitive learning for topographic mapping

• Merkl & Rauber (1997) "Alternative Ways for Cluster Visualization in SOM"
  - Advanced techniques for hierarchical visualization and interpretation
  - Multi-level clustering strategies for complex data structures

╔════════════════════════════════════════════════════════════════════════════════╗
║                                 ELI5 SUMMARY                                   ║
╚════════════════════════════════════════════════════════════════════════════════╝

🧠 Think of your brain's visual cortex - it processes images at multiple scales!

Imagine looking at a forest 🌲:
• LEVEL 1: "I see a forest" (coarse, overview)
• LEVEL 2: "I see trees, rocks, paths" (medium detail)  
• LEVEL 3: "I see leaves, bark texture, pebbles" (fine detail)

Hierarchical SOMs work the same way:
1. 🗺️  TOP LEVEL: Creates a rough map of major data clusters
2. 🔍 MIDDLE LEVELS: Each cluster grows its own detailed sub-map
3. 🧩 BOTTOM LEVEL: Fine-grained patterns within each sub-region

Like having a world map → country map → city map → street map!
Each level "zooms in" on interesting regions discovered by the level above.
...
╚════════════════════════════════════════════════════════════════════════════════╝
    ...
    Level 1 (Children):    SOM-A    SOM-B    SOM-C
                          ┌───┬───┐ ┌───┬───┐ ┌───┬───┐
                          │A1 │A2 │ │B1 │B2 │ │C1 │C2 │
                          ├───┼───┤ ├───┼───┤ ├───┼───┤
                          │A3 │A4 │ │B3 │B4 │ │C3 │C4 │
    ...
    Level 2 (Grandchildren): SOM-A1 SOM-A2 ...   (even finer detail)

    TRAINING FLOW:
    Data → Level 0 learns → Create children for active regions →
    Data subset → Level 1 learns → Create grandchildren → Continue...
...
1. WINNER SELECTION at Level L:
   BMU_L(x) = argmin_i ||x - w_i^(L)||²

2. HIERARCHICAL WEIGHT UPDATE:
   w_i^(L)(t+1) = w_i^(L)(t) + α_L(t) · h_L(BMU, i, t) · (x - w_i^(L)(t))
   
   Where: α_L(t) = α₀ · decay_rate^L  (learning rate decreases with depth)

3. EXPANSION CRITERION:
   Create_Child(node_i) = |Data_i| ≥ min_threshold ∧ Variance(Data_i) ≥ var_threshold

4. HIERARCHICAL DISTANCE:
   d_hier(x,y) = Σ(L=0 to max_depth) w_L · d_L(BMU_L(x), BMU_L(y))

🎯 KEY PARAMETERS:
• expansion_factor: How much bigger child SOMs are than parents
• learning_rate_decay: How quickly learning slows at deeper levels  
• min_data_per_node: Minimum data points needed to justify expansion
• expansion_threshold: Variance threshold for creating child layers
...
1. 🖼️  COMPUTER VISION:
   - Multi-scale image analysis (satellite → building → room → objects)
   - Hierarchical object recognition with scale invariance
   - Medical imaging: organ → tissue → cellular → molecular levels

2. 📊 DATA MINING & ANALYTICS:
   - Customer segmentation: demographic → behavioral → transactional patterns
   - Market research: industry → company → product → feature clustering
   - Time series analysis: yearly → monthly → daily → hourly patterns

3. 🧬 BIOINFORMATICS:
   - Gene expression analysis: pathway → gene family → individual genes
   - Protein structure: domain → motif → amino acid sequence patterns
   - Phylogenetic tree construction with multi-level evolutionary relationships

4. 🌐 INTERNET & SOCIAL MEDIA:
   - Web page categorization: topic → subtopic → specific content
   - Social network analysis: communities → groups → individual connections
   - Search result organization: category → subcategory → specific pages

5. 🏭 INDUSTRIAL APPLICATIONS:
   - Manufacturing quality control: system → component → defect patterns
   - Supply chain optimization: region → facility → process → resource level
   - Predictive maintenance: equipment → subsystem → component failure patterns

The hierarchical approach is especially powerful for:
• Handling datasets with natural multi-scale structure
• Providing interpretable results at multiple levels of detail
• Scaling to very large datasets through divide-and-conquer
• Discovering both global trends AND local patterns simultaneously
...
class HierarchicalSOM:
    ...
    def prune_empty_branches(self, min_activation_threshold: float = 0.01):
        """
        Remove layers/SOMs that do not contribute meaningfully to the hierarchy.
    ...
    def _recompute_hierarchy_structure(self):
        ...
        print(f"Hierarchy structure recomputed: {len(self.levels)} levels remaining")
    ...
    def get_cluster_hierarchy(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        for level, repr_vec in representations.items():
            ...
            unique_reprs = np.unique(repr_vec, axis=0)
            level_clusters = {}
            
            for i, unique_repr in enumerate(unique_reprs):
                # Find all data points with this representation
                matches = np.all(repr_vec == unique_repr, axis=1)
                cluster_data = data[matches]
                
                level_clusters[f"cluster_{i}"] = {
                    'data_indices': np.where(matches)[0],
                    'centroid': np.mean(cluster_data, axis=0),
                    'size': len(cluster_data),
                    'representation': unique_repr
                }
        ...
        return {
            'clusters': clusters,
            'total_levels': len(clusters),
            'hierarchy_depth': max(representations.keys()) if representations else 0
        }